...
my $DP_PAR1_TOTAL = 10; #
my $DP_PAR2_TOTAL = 10; #
$DP_PAR1_TOTAL = $CONF{'P1.Depth'} if(exists $CONF{'P1.Depth'});
$DP_PAR2_TOTAL = $CONF{'P2.Depth'} if(exists $CONF{'P2.Depth'});

# global options: upper depth
my $DP_PAR1_UPPER = $CONF{'P1.Depth_upper'} if(exists $CONF{'P1.Depth_upper'});
my $DP_PAR2_UPPER = $CONF{'P2.Depth_upper'} if(exists $CONF{'P2.Depth_upper'});

# global options: lower depth
my $TOTAL_DEPTH = $CONF{'Total.Depth'} if(exists $CONF{'Total.Depth'});

# global options: upper depth
my $TOTAL_DEPTH_UPPER = $CONF{'Total.Depth_upper'} if(exists $CONF{'Total.Depth_upper'});
...
$P1GT_WRONG =  $CONF{'P1.GT_WRONG'} if(exists $CONF{'P1.GT_WRONG'});
$P2GT_WRONG =  $CONF{'P2.GT_WRONG'} if(exists $CONF{'P2.GT_WRONG'});
$P1GT_RIGHT =  $CONF{'P1.GT_RIGHT'} if(exists $CONF{'P1.GT_RIGHT'});
$P2GT_RIGHT =  $CONF{'P2.GT_RIGHT'} if(exists $CONF{'P2.GT_RIGHT'});

#
say "conditions:";
say "DP_PAR1_TOTAL     = $DP_PAR1_TOTAL"       if(exists $CONF{'P1.Depth'});
say "DP_PAR2_TOTAL     = $DP_PAR2_TOTAL"       if(exists $CONF{'P2.Depth'});
...
while(<FILE>){
	...
	if($_=~m/^#(.*)/){
		...
		die "VCF file error!!!Parent1 missing.\n" if( &check_samples(\%samples, \@PARENT1) );
		die "VCF file error!!!Parent2 missing.\n" if( &check_samples(\%samples, \@PARENT2) );
		# write out
		print OUT "$_\n";
		next;
	}
